...
client.once('ready', ()=> {
    ...
    client.user.setActivity("If I were you, I'd hang myself:)")
});
...
client.on('message', message =>{
    ...
    if (command==='abel'){
      ...
      client.commands.get('derjanko').execute(message, args);
  }   else if(command==='keressukmeg'){
      client.commands.get('keressukmeg').execute(message, args);
...
client.login('ODMxNTUyOTc2NDMwMDM5MDgy.YHW6Dw.8GMZrsXH_u9URoNej0-kPlHCQHk');
